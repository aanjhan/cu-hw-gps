...
in_sig_2ms = in_sig(sigOffset+(1:floor((codeLength+1)*ONE_MSEC_SAM)));      %Use only 2 msec of input data

%create the upsampled CA Code for this specific satellite
CAcode = digitize_ca_prompt(WAASCODEGN(PRN),codeLength);
...
for fd=-FD_SIZE:FREQ_STEP:FD_SIZE           
    ...
    %demodulate the input signal at the current doppler shift
    Si = in_sig_2ms.*AMP.*cos(2*pi*freq_argument*time);
    Sq = -in_sig_2ms.*AMP.*sin(2*pi*freq_argument*time);
    
    %Do the cross correlations to get the values
    Icacorr(:,freq_bin) = xcorr(Si,CAcode);
    Qcacorr(:,freq_bin) = xcorr(Sq,CAcode);
    
    %Find I^2+Q^2
    I2Q2(:,freq_bin) = Icacorr(:,freq_bin).^2+Qcacorr(:,freq_bin).^2;
