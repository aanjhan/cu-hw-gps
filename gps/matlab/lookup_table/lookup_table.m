function table=lookup_table(type,quantization,width,module)
    num_entries=2^width;
    ...
    if(~isempty(module))
        ...
        f=fopen(sprintf('%s.csv',module),'wt');
        ...
        fprintf(f,'    output wire [`%s_OUTPUT_RANGE] out);\n',const);
        fprintf(f,'\n');
        fprintf(f,'   always @(in) begin\n');
        for i=1:num_entries
            fprintf(f,'      case `%s_INPUT_WIDTH\''d%d: out <= `%s_OUTPUT_WIDTH\''d%d;\n',...
                const,i-1,...
                const,table(i));
        end
        fprintf(f,'   end\n');
